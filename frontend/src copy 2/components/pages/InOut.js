...
function Home() {
  ...
  const getEmployeeData = () => {
    axios.get(baseUrl + "employees").then((res) => {
      setEmployeesList(res.data.data.filter((i)=>i.aid!==""));
    });
  };
  ...
  const postTin = (y) => {
    ...
    axios.put(baseUrl + "attendance/tin/"+y, item).then((res)=>axios.put(baseUrl+"employees/inout/"+res.data.data.empId,{aid:y,status:"in"})).then(()=>getEmployeeData())
  };
  const postTout = (y) => {
    const item={
      tOut:Date.now(),
    ...
    axios.put(baseUrl + "attendance/tout/"+y, item).then((res)=>axios.put(baseUrl+"employees/inout/"+res.data.data.empId,{aid:y,status:"out"})).then(()=>getEmployeeData())
  };
