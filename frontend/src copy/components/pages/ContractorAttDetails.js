...
function Home() {
  ...
  .filter((j) => j.contractorName.includes(name?.contractorName) & (j.status === "Present"))
  .map((j) => +j.date);
const absentCheck = list
  .filter((j) => j.contractorName.includes(name?.contractorName) & (j.status === "Absent"))
  .map((j) => +j.date);
const halfdayCheck = list
  .filter((j) => j.contractorName.includes(name?.contractorName) & (j.status === "Halfday"))
  .map((j) => +j.date);
    ...
    <div class="modal-content">
      ...
      <div className="modal11">
            ...
            >
              ...
              {name?.contractorName}'s Celender
              </label>
            </div>
            <div
              style={{
                        ...
                        <tbody>
                          {empList.map((i, n) => (
                            <tr>
                              ...
                              <td>{list.filter((j)=>j.contractorName===i.contractorName&j.status==="Present").length}</td>
                              <td>{list.filter((j)=>j.contractorName===i.contractorName&j.status==="Absent").length}</td>
                              <td>{list.filter((j)=>(j.contractorName===i.contractorName)&(j.status==="Halfday")& (j.month==currMonth&j.year==currYear)).length}</td>
