...
function Home() {
  ...
  const getData = () => {
    axios.get(baseUrl + "labouratt").then((res) => setList(res.data.data));
  };

  const getLabourData = () => {
    axios.get(baseUrl + "labour").then((res) => setEmpList(res.data.data));
  };
  ...
  .filter((j) => j.labourName?.includes(name?.labourName) & (j.status === "Present"))
  .map((j) => +j.date);
const absentCheck = list
  .filter((j) => j.labourName?.includes(name?.labourName) & (j.status === "Absent"))
  .map((j) => +j.date);
const halfdayCheck = list
  .filter((j) => j.labourName?.includes(name?.labourName) & (j.status === "Halfday"))
  .map((j) => +j.date);
                        ...
                        <tbody>
                          {empList.map((i, n) => (
                            <tr>
                              ...
                              <td>{list.filter((j)=>j.labourName===i.labourName&j.status==="Present").length}</td>
                              <td>{list.filter((j)=>j.labourName===i.labourName&j.status==="Absent").length}</td>
                              <td>{list.filter((j)=>(j.labourName===i.labourName)&(j.status==="Halfday")& (j.month==currMonth&j.year==currYear)).length}</td>
                              <td><i 
                              onClick={()=>setName(i)} 
                              data-bs-toggle="modal" data-bs-target="#exampleModal" className="fa fa-eye"></i></td>
