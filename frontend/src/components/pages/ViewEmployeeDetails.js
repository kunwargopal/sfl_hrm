...
function Home() {
  ...
  const attendanceLocalData = sessionStorage.getItem("attendanceLocalData")
  const parseAttendance = attendanceLocalData?JSON.parse(attendanceLocalData):[]
  const [attList, setAttList] = useState(parseAttendance);

  const getAttendanceData = () => {
    axios.get(baseUrl + "attendance").then((res) =>{ setAttList(res.data)
    sessionStorage.setItem("attendanceLocalData", JSON.stringify(res.data))
    });
  };

  const list = attList.filter((i) => i.empId === employeeData.id);
  ...
  const allDepartment=()=>{
  axios.get(baseUrl + "department").then((res)=>{setDepartmentList(res.data)
  sessionStorage.setItem("departmentLocalData", JSON.stringify(res.data))
  })
  }
  const allDesignation=()=>{
  axios.get(baseUrl + "designation").then((res)=>{setDesignationList(res.data)
    sessionStorage.setItem("designationLocalData", JSON.stringify(res.data))

  })
  }

const getDepartmentName =(x)=>{
  const department = departmentList.filter((i)=>i.id===x)
  const dep = department.length?department[0].department:""
  return dep
}
const getDesignationName =(x)=>{
  const designation = designationList.filter((i)=>i.id===x)
  const des = designation.length?designation[0].designation:""
  return des
}
                          ...
                          <div className="row g-3">
                            ...
                            <div className="col-md-6">
                              <div className="row">
                                ...
                                <div className="col-sm-8">
                                  <input
                                    value={getDepartmentName(employeeData.department)}
                            ...
                            <div className="col-md-6">
                              <div className="row">
                                ...
                                <div className="col-sm-8">
                                  <input
                                    value={getDesignationName(employeeData.designation)}
                      ...
                      >
                        ...
                        <div className="accordion-body">
                          <div className="row g-3">
                            ...
                            {dayList.map((i) => (
                              <button  disabled={new Date(getDate1(i))>new Date(currDate)} onClick={()=>{
                              ...
                              }} className={`btn ${getDate1(i)===currDate?"btn-outline-primary":prList.includes(getDate1(i))?"btn-outline-success":new Date(getDate1(i))<new Date(currDate)?"btn-outline-danger":"btn-outline-secondary"} m-1`}
                              style={{width:70,borderWidth:2}}>
                                {i}
                              </button>
                            ))}
                        </div>
