...
function Home() {
  ...
  const list2 =
    department !== ""
      ? list.filter((i) => i.department === department)
      : list;
  const list3 =
    designation !== ""
...
putAttandance(url,y)
  ...
  const postAttandance = (x,y) => {
    ...
    axios.post(baseUrl + "attendance", item).then((res)=>axios.patch(baseUrl+"employees/"+y,{aid:res.data.id,status:"in"})).then(()=>getEmployeeData())
  };
  const putAttandance = (x,y) => {
    ...
    axios.patch(baseUrl + "attendance/"+y, item).then((res)=>axios.patch(baseUrl+"employees/"+res.data.empId,{aid:"",status:""})).then(()=>getEmployeeData())
  };
                      ...
                      <tbody>
                        {list4.map((i, n) => (
                          <tr>
                            ...
                            <td>
                            <input
                              onChange={(e) => uploadImage(e.target.files[0],i.id)}
                              id={"in"+n}
                              style={{ display: "none" }}
                              type="file"
                            />
